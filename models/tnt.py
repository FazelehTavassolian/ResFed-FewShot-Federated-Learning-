...
class Block(nn.Module):
    def __init__(self, outer_dim, inner_dim, outer_num_heads, inner_num_heads, num_words, mlp_ratio=4., qkv_bias=False,
                 qk_scale=None, drop=0., attn_drop=0., drop_path=0., act_fn=nn.GELU, norm_layer=nn.LayerNorm, se=0):
        super().__init__()
        ...
        self.outer_mlp = Mlp(in_features=outer_dim, hidden_features=int(outer_dim * mlp_ratio),
                             out_features=outer_dim, act_layer=act_fn, drop=drop)
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_channels=3, outer_dim=768, inner_dim=24, inner_stride=4):
        super().__init__()
        ...
        self.proj = nn.Conv2d(in_channels, inner_dim, kernel_size=7, padding=3, stride=inner_stride)
...
class TNT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, outer_dim=768, inner_dim=48,
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_channels=in_chans, outer_dim=outer_dim,
...
def tnt_s_patch16_224(pretrained=False, **kwargs):
    ...
    pretrained_url = None
    if kwargs.get('pretrain_url',None) is not None:
        pretrained_url = kwargs.pop('pretrain_url')
    ...
    if pretrained:
        ...
        print('[*] Pretrained model had been loaded!')
    return model
...
#
#     model = tnt_s_patch16_224(pretrained=True, num_classes=5)
#     state_dict = torch.load('/home/jericho/Project/federated-meta-learning/Data/models/tnt_s_patch16_224.pth')
#     state_dict.pop('head.weight')
#     state_dict.pop('head.bias')
#     model.load_state_dict(state_dict,strict=False)
