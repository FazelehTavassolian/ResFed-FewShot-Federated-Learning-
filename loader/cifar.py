...
def get_pure_ci_far(n_classes: int = 100, cache_dir='./Data', resize=224, verbose: bool = False) -> DatasetFmt:
    """
    Get Cif-ar dataset
    """
    if n_classes == 10:
    ...
    train_data = dataset(root=cache_dir, train=True,
                         download=True, transform=train_transform)

    test_data = dataset(root=cache_dir, train=False,
                        download=True,
...
def get_episodic_ci_far100(args,fs, **kwargs) -> DatasetFmt:
    # Make Augmentation
    ...
    train_ds = CiFarDataset(img_dir=ds_root.joinpath('train'))
